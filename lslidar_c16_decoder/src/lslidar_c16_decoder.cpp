...
{
  LslidarC16Decoder::LslidarC16Decoder(ros::NodeHandle &n, ros::NodeHandle &pn)
      : nh(n), pnh(pn), publish_point_cloud(true), is_first_sweep(true),
        last_azimuth(0.0), sweep_start_time(0.0),
        // layer_num(8),
  ...
  bool LslidarC16Decoder::loadParameters()
  {
    ...
    pnh.param<int>("ground_point_min_laser_index", ground_point_min_laser_index,
                   0);
    pnh.param<int>("ground_point_max_laser_index", ground_point_max_laser_index,
                   7);
    pnh.param<double>("ground_point_min_angle", ground_point_min_angle, 90);
    ROS_INFO("Load paramter: ground_point_min_angle :%f ", ground_point_min_angle);

    ground_point_min_angle = ground_point_min_angle / 180.0 * M_PI;

    pnh.param<double>("ground_point_distance_tolerance",
                      ground_point_distance_tolerance, 0.05);
  ...
  bool LslidarC16Decoder::createRosIO()
  {
    ...
    layer_sub =
        nh.subscribe("layer_num", 100, &LslidarC16Decoder::layerCallback, this);
    sweep_pub =
        nh.advertise<lslidar_c16_msgs::LslidarC16Sweep>("lslidar_sweep", 10);
    point_cloud_pub =
        nh.advertise<sensor_msgs::PointCloud2>("lslidar_point_cloud", 10);
  ...
  void LslidarC16Decoder::publishPointCloud()
  {
    ...
    for (size_t i = 0; i < 16; ++i)
    {
      ...
      double circle_start_distance = sqrt(scan.points[1].x * scan.points[1].x +
                                          scan.points[1].y * scan.points[1].y);
      size_t circle_start_index = 0;
      double circle_start_azimuth = scan.points[0].azimuth;

      // ROS_INFO("start angle: %lf  ", circle_start_azimuth);
      ...
      for (j = 1; j < scan.points.size() - 1; ++j)
      {
        ...
        {
          double pt_distance = sqrt(scan.points[i].x * scan.points[j].x +
                                    scan.points[j].y * scan.points[j].y);
          // ROS_INFO("d distance: %f", abs(pt_distance - circle_start_distance)) ;

          if ((abs(pt_distance - circle_start_distance) <
               ground_point_distance_tolerance) &&
              (j < (scan.points.size() - 2)))
          {
          }
          else
          {
            // ROS_INFO("out of tolerance angle :%f thresh angle: %lf ",abs(scan.points[j].azimuth - circle_start_azimuth) ,ground_point_min_angle) ;

            if (abs(scan.points[j].azimuth - circle_start_azimuth) >
                ground_point_min_angle)
            {
              // ROS_INFO("delete point from %ld to %ld", circle_start_index, j);
              point_cloud->points.resize(point_cloud->points.size() +
                                         circle_start_index - j);
              point_cloud->width += circle_start_index - j;
            }

            circle_start_index = j;
            circle_start_distance = pt_distance;
            circle_start_azimuth = scan.points[j].azimuth;
          }
        }
      }

      // ROS_INFO("end angle: %lf  ", scan.points[j].azimuth);
    }
  ...
  void LslidarC16Decoder::publishChannelScan()
  {
    ...
    // lslidar_c16_msgs::LslidarC16Layer multi_scan(new
    ...
    for (uint16_t j = 0; j < 16; j++)
    {
      ...
      for (uint16_t i = 0; i < sweep_data->scans[j].points.size(); i++)
      {
        ...
        scan.ranges[point_num - 1 - point_idx] =
            sweep_data->scans[j].points[i].distance;
        scan.intensities[point_num - 1 - point_idx] =
            sweep_data->scans[j].points[i].intensity;
      }
      ROS_INFO("Point num: %d ", point_num);
      for (int i = point_num - 1; i >= 0; i--)
      {
        if ((i >= angle_disable_min * point_num / 360.0) &&
            (i < angle_disable_max * point_num / 360.0))
  ...
  void LslidarC16Decoder::publishScan()
  {
    ...
    for (uint16_t i = 0; i < sweep_data->scans[layer_num_local].points.size();
         i++)
    {
      int point_idx =
          sweep_data->scans[layer_num_local].points[i].azimuth / angle_base;
  ...
  void LslidarC16Decoder::decodePacket(const RawPacket *packet)
  {
    ...
    //{
    //	ROS_WARN("[%f %f %f]", firings[fir_idx].azimuth[0],
    // firings[fir_idx].distance[0], firings[fir_idx].intensity[0]);
    //}
    return;
  }
      ...
      const lslidar_c16_msgs::LslidarC16PacketConstPtr &msg)
  {
    ...
    for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx)
    {
      for (size_t scan_idx = 0; scan_idx < SCANS_PER_FIRING; ++scan_idx)
      {
        ...
        lslidar_c16_msgs::LslidarC16Point
            &new_point = // new_point 为push_back最后一个的引用
