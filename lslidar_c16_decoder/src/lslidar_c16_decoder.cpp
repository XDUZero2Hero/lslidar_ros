...
void LslidarC16Decoder::publishPointCloud() {
  ...
  for (size_t i = 0; i < 16; ++i) {
    ...
    size_t j;
    VPoint point;
    static double ground_point_distance_tolerance_2 =
        ground_point_distance_tolerance * ground_point_distance_tolerance;
    double circle_start_distance = scan.points[0].x * scan.points[0].x +
                                   scan.points[0].y * scan.points[0].y;
    ...
    for (j = 1; j < scan.points.size() - 1; ++j) {
      ...
      double pt_distance = scan.points[i].x * scan.points[j].x +
                           scan.points[j].y * scan.points[j].y;
      if (abs(pt_distance - circle_start_distance) <
          ground_point_distance_tolerance_2) {
      } else {
        if (abs(scan.points[j].azimuth - circle_start_azimuth) >
            ground_point_min_angle) {
          ROS_INFO_ONCE("delete point from %d to %d", circle_start_index, j);
